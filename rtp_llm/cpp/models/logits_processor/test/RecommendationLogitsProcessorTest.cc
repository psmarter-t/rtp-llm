...
static SamplerInputs allocateSamplerInputs(size_t                 batch_size,
    ...
    LogitsProcessorStatesPtr state_ptr = std::make_shared<LogitsProcessorStates>(std::move(output_vocab_mapping));
...
TEST_F(RecommendationLogitsProcessorTest, testProcessMapsFullBannedTokenToPrunedLogitsColumn) {
    auto info           = makeInfo(2, {{2, 7}});
    info.pos_in_combo   = 1;
    ...
    EXPECT_FLOAT_EQ(logits[0][0].item<float>(), 1.0f);
    EXPECT_FLOAT_EQ(logits[0][1].item<float>(), 1.0f);
    const auto masked_logit = logits[0][2].item<float>();
    EXPECT_TRUE(std::isinf(masked_logit));
    EXPECT_LT(masked_logit, 0.0f);
}
