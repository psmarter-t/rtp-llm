    ...
    if (need_ban_process) {
        ...
        for (size_t i = 0; i < batch_size; ++i) {
            ...
            for (const auto& combo : info.banned_combos) {
                ...
                const auto logits_token_id = toLogitsTokenId(banned_token);
                if (logits_token_id.has_value() && static_cast<size_t>(logits_token_id.value()) < vocab_size) {
                    rows.push_back(static_cast<int64_t>(i));
                    cols.push_back(static_cast<int64_t>(logits_token_id.value()));
                }
            }
        }
        if (!rows.empty()) {
